...
/**
 * Regex that matches valid avatar hashes. Hashes are alphanumeric and lowercase, optionally starting with 'a_'. I
 * couldn't find Discord documentation on hash formatting but it appears to follow this pattern. Pattern is
 * /^(a_)?[a-z0-9]+$/
...
/**
 * Regex that matches valid snowflakes. Snowflakes are strings of only digits and are between 17 and 20 digits,
 * inclusive. 17 because the oldest id on Discord is 17 digits. 20 because snowflakes are up to 64 bits and the max
 * value of a 64 bit number is 20 digits. Pattern is /^\d{13,20}$/
...
/**
 * Regex that matches valid Discord usernames. Usernames are between 2 and 32 characters long, do not contain leading or
 * trailing whitespace, do not contain the following substrings: [@, #, :, ```, discord], and are not 'everyone' or
 * 'here' (no quotes). Pattern is /^[^@#:\s][^@#:]{0,30}[^@#:\s](?<!^(?:everyone|here)|.*(?:```|discord).*)$/
...
// TODO: test s option at the end so you don't have to do [\s\S]?
